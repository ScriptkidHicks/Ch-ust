...
pub fn test_en_passant_should_pass() {
    ...
    altered_white_advance_row.set_square(3, Square::Full(Piece{color: PieceColor::White, kind: PieceKind::Pawn}));
    ...
    let current_black_pawn_square_coords = Coordinates {letter: ColumnLetter::E, number: 7};
    let black_pawn_jump_target_coords = Coordinates {letter: ColumnLetter::E, number: 5};

    let mut current_turn_board = previous_turn_board.clone();
    
    let move_result = current_turn_board.move_piece(&current_black_pawn_square_coords, &black_pawn_jump_target_coords, None);
    ...
    let white_pawn_from_square = Coordinates {
        letter: ColumnLetter::D, number: 5
    };
    let white_pawn_to_square = Coordinates {
        letter: ColumnLetter::E, number: 6
    };
    let en_passant_move_result = current_turn_board.move_piece(&white_pawn_from_square, &white_pawn_to_square, Some(&previous_turn_board));
