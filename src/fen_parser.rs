...
use crate::{board::{Board, Row}, pieces::{Piece, PieceColor, PieceKind}};
...
pub fn parse_char_to_piece(input_char: char) -> Option<Piece> {
    ...
    let piece_color: PieceColor = if input_char == lowercased_input {PieceColor::Black} else {PieceColor::White};

    match parse_char_to_piecekind(lowercased_input) {
        Some(piece_kind) => {
            Some(Piece {kind: piece_kind, color: piece_color})
        },
        None => {
            None
        }
